...
const About = ({ data }) => {
  const { details, cvPdf } = data;

  const contextValues=useContext(NoteContext)
   if (!contextValues || !contextValues.apidata || !contextValues.apidata.success || !contextValues.apidata.user || !contextValues.apidata.user.about) {
    return <p>Error: Unable to retrieve user data</p>;
}

const { url } = contextValues.apidata.user.about.avatar;
const { name,title,subtitle,description } = contextValues.apidata.user.about;
            ...
            <div className="st-vertical-middle">
              <div className="st-vertical-middle-in">
                <div className={`st-text-block st-style1`} data-aos="zoom-in" data-aos-duration="1000" data-aos-delay="500">
                  <h2 className="st-text-block-title">{name}</h2>
